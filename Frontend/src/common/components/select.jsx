...
function Select({ label, options, variant = "primary", error, ...props }) {
  ...
  return (
    <div className="w-full text-left">
      {label && (
        <label className={`block text-sm font-medium mb-1 ${variant === 'auth' ? 'text-white' : 'text-gray-700'}`}>
          {label}
        </label>
      )}
      
      <select
        className={`${styles} w-full border p-2.5 outline-none transition-all cursor-pointer shadow-sm focus:ring-2`}
        {...props}
      >
