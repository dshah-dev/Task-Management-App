...
function Input({ variant = "primary", label, error, ...props }) {
  const variants = {
    primary: {
      input: "w-full border rounded p-2 bg-white outline-none focus:ring-2 focus:ring-blue-500",
      label: "text-black"
    },
    searchBar: {
  ...
  return (
    <div className="mb-4">
      {label && (
        <label
          className={`${activeVariant.label} block text-sm mb-1`}
        >
          {label}
        </label>
      )}
